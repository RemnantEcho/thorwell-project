...
/**
 * An BufferedInputStream that parses Shoutcast's "icy" metadata from the
 * stream. Gets headers at the beginning and if the "icy-metaint" tag is found,
 * it parses and strips in-stream metadata.
 * <p>
 * <b>The deal with metaint</b>: Icy streams don't try to put tags between MP3
 * frames the way that ID3 does. Instead, it requires the client to strip
 * metadata from the stream before it hits the decoder. You get an
 * <code>icy-metaint</code> name/val in the beginning of the stream iff you sent
 * "Icy-Metadata" with value "1" in the request headers (SimpleMP3DataSource
 * does this if the "parseStreamMetadata" boolean is true). If this is the case
 * then the value of icy-metaint is the amount of real data between metadata
 * blocks. Each block begins with an int indicating how much metadata there is
 * -- the block is this value times 16 (it can be, and often is, 0).
 * <p>
 * Originally thought that "icy" implied Icecast, but this is completely wrong
 * -- real Icecast servers, found through www.icecast.net and typified by URLs
 * with a trailing directory (like CalArts School of Music -
 * http://65.165.174.100:8000/som) do not have the "ICY 200 OK" magic string or
 * any of the CRLF-separated headers. Apparently, "icy" means "Shoutcast". Yep,
 * that's weird.
 *
...
public class IcyInputStream
    ...
    HashMap tags;
    /**
     * Buffer for readCRLF line... note this limits lines to 1024 chars (I've
     * read that WinAmp barfs at 128, so this is generous)
     */
    protected byte[] crlfBuffer = new byte[1024];
    /**
     * value of the "metaint" tag, which tells us how many bytes of real data
     * are between the metadata tags. if -1, this stream does not have metadata
     * after the header.
     */
    protected int metaint = -1;
    /**
     * how many bytes of real data remain before the next block of metadata.
     * Only meaningful if metaint != -1.
     */
    protected int bytesUntilNextMetadata = -1;
    // TODO: comment for constructor

    /**
     * Reads the initial headers of the stream and adds tags appropriatly. Gets
     * set up to find, read, and strip blocks of in-line metadata if the
    ...
    /**
     * Assuming we're at the top of the stream, read lines one by one until we
     * hit a completely blank \r\n. Parse the data as IcyTags.
     */
    ...
    /**
     * Reads and returns a single byte. If the next byte is a metadata block,
     * then that block is read, stripped, and parsed before reading and
     * returning the first byte after the metadata block.
     */
    ...
    /**
     * Reads a block of bytes. If the next byte is known to be a block of
     * metadata, then that is read, parsed, and stripped, and then a block of
     * bytes is read and returned. Otherwise, it may read up to but not into the
     * next metadata block if <code>bytesUntilNextMetadata &lt; length</code>
     */
     ...
     * be right on the segment, ie, the next byte to read is the metadata block
     * count. The metadata is parsed and new tags are added with addTag(), which
     * fires events
     */
    ...
    /**
     * Parse metadata from an in-stream "block" of bytes, add a tag for each
     * one.
     * <p>
     * Hilariously, the inline data format is totally different than the
     * top-of-stream header. For example, here's a block I saw on "Final Fantasy
     * Radio":
     * <pre>
    ...
    /**
     * adds the tag to the HashMap of tags we have encountered either in-stream
     * or as headers, replacing any previous tag with this name.
     */
    ...
    /**
     * Get the named tag from the HashMap of headers and in-line tags. Null if
     * no such tag has been encountered.
     */
    ...
    /**
     * Removes a TagParseListener, so it won't be notified when this stream
     * parses MP3Tags.
     */
